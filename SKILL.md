...
## Creating a PHP C Extension - Step by Step Guide

This section provides a practical, hands-on guide to creating a custom PHP extension in C from scratch.

### 1. Directory Structure

A well-organized PHP extension follows this standard directory structure:

```
my_extension/
├── config.m4              # Autoconf configuration for Unix/Linux/macOS
├── config.w32             # Windows build configuration
├── php_my_extension.h     # Public header file (exposed to PHP userspace)
├── my_extension.c         # Main implementation file
├── php_my_extension_int.h # Internal header (not exposed)
├── my_extension_stub.c    # Stub for static compilation
├── arginfo/               # Automatically generated arginfo files
│   └── my_extension_arginfo.h
└── tests/                 # Test files
    ├── phpt/              # PHPT test files
    │   ├── basic.phpt
    │   ├── edge_cases.phpt
    │   └── ...
    └── php/               # PHP-based tests (PestPHP, PHPUnit)
        ├── pest.php
        ├── PestTest.php
        └── ...
```
...
**File Purpose:**
- `config.m4` - Autoconf macro for Unix build system integration
- `config.w32` - Windows-specific build configuration
- `php_my_extension.h` - Public API declarations, function prototypes exposed to PHP
- `my_extension.c` - Function implementations, module entry point
- `php_my_extension_int.h` - Internal structures, types, and helper functions
- `my_extension_stub.c` - Empty stub for static builds
- `arginfo/` - Contains arginfo structures generated by `ext_skel` or similar tools
- `tests/` - Test suite directory

### 2. Stubs to Automatically Generate arginfo

PHP 7.4+ introduced a new arginfo system that replaces the old `ZEND_BEGIN_ARG_INFO` macros with a more efficient approach. Use the `ext_skel` script or manually create arginfo stubs.

**Using ext_skel (PHP 7.4+):**

The `ext_skel` script generates boilerplate code including arginfo:
...
./ext_skel --extname=my_extension --proto=my_extension.proto
```

**Manual arginfo Generation:**

Create `arginfo/my_extension_arginfo.h`:

```c
/* Automatically generated by ext_skel or manually created */

ZEND_BEGIN_ARG_INFO_EX(arginfo_my_extension_hello, 0, 0, 1)
    ZEND_ARG_INFO(0, name)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_my_extension_add, 0, 0, 2)
    ZEND_ARG_INFO(0, a)
    ZEND_ARG_INFO(0, b)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_my_extension_class_method, 0, 0, 1)
    ZEND_ARG_INFO(0, param)
ZEND_END_ARG_INFO()
```

**In your main C file (my_extension.c):**

```c
#include "php_my_extension.h"
#include "arginfo/my_extension_arginfo.h"

// Function declaration with arginfo
...
// Function entry with arginfo
static const zend_function_entry my_extension_functions[] = {
    PHP_FE(my_extension_hello,    arginfo_my_extension_hello)
    PHP_FE(my_extension_add,      arginfo_my_extension_add)
    PHP_FE_END
};
```

**Modern arginfo with ZEND_BEGIN_ARG_WITH_RETURN_TYPE (PHP 8.0+):**

```c
ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_my_extension_add, 0, 2, IS_LONG, 0)
    ZEND_ARG_TYPE_INFO(0, a, IS_LONG, 0)
    ZEND_ARG_TYPE_INFO(0, b, IS_LONG, 0)
ZEND_END_ARG_INFO()
```

This provides type hints in reflection and better IDE support.

**Generating arginfo from prototypes:**

Create a `my_extension.proto` file:
```
// Function prototypes
long my_extension_add(long a, long b)
string my_extension_hello(string name)
```

Then use tools like `phpize` or custom scripts to generate arginfo from these prototypes.

### 3. PHPT Tests

PHPT (PHP Test) files are the standard way to test PHP extensions and core functionality.

**PHPT File Structure:**

```phpt
--TEST--
Test my_extension_hello function
--DESCRIPTION--
Tests that my_extension_hello returns expected output
--SKIPIF--
<?php if (!extension_loaded('my_extension')) die('skip'); ?>
--INI--
; Any INI settings needed
my_extension.some_setting=1
--FILE--
<?php
// Test code here
echo my_extension_hello('World');
--EXPECT--
Hello, World!
--EXPECTF--
; For flexible output matching (regex)
/^Hello, \w+!/
--CLEAN--
; Cleanup code if needed
<?php my_extension_cleanup(); ?>
```

**Common PHPT Test Types:**

1. **Basic Functionality:**
```phpt
--TEST--
my_extension_add basic test
--FILE--
<?php
var_dump(my_extension_add(2, 3));
--EXPECT--
int(5)
```
...
```phpt
--TEST--
my_extension_add with invalid argument
--FILE--
<?php
try {
    my_extension_add("not a number", 3);
} catch (TypeError $e) {
    echo "Caught TypeError: " . $e->getMessage();
}
--EXPECT--
Caught TypeError: Argument #1 ($a) must be of type int, string given
```

3. **INI Settings:**
```phpt
--TEST--
my_extension with custom INI setting
--INI--
my_extension.timeout=30
--FILE--
<?php
var_dump(ini_get('my_extension.timeout'));
--EXPECT--
string(2) "30"
```
...
cd /path/to/php-src/ext/my_extension
make test

# Run specific test
php -n -d extension=my_extension.so run-tests.php tests/phpt/hello.phpt

# Run tests with valgrind (memory leak detection)
MAKE_TESTS="-m --valgrind" make test

# Run tests with specific PHP executable
TEST_PHP=./sapi/cli/php run-tests.php tests/
```

**PHPT Test Tips:**
- Use `--FILE--` for the main test code
- Use `--SKIPIF--` to skip tests when requirements aren't met
- Use `--EXPECT--` for exact output matching
- Use `--EXPECTF--` for flexible output matching with regex
- Use `--INI--` to set PHP configuration for the test
- Use `--CLEAN--` for cleanup code

### 4. Writing PHP Tests with PestPHP Framework

While PHPT tests are essential for the extension itself, you can also write userland PHP tests using PestPHP for easier testing during development.

**Setup PestPHP:**

```bash
# In your extension's tests/php directory
cd /path/to/my_extension/tests/php
composer init
...
**Example PestPHP Test File:**

```php
<?php

// tests/php/PestTest.php
...
class MyExtensionTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        if (!extension_loaded('my_extension')) {
            $this->markTestSkipped('my_extension extension not loaded');
        }
    }
    ...
    {
        ob_start();
        my_extension_hello('Pest');
        $output = ob_get_clean();
        
        $this->assertEquals('Hello, Pest!' . PHP_EOL, $output);
    }

    public function testAddFunction()
    {
        $this->assertEquals(5, my_extension_add(2, 3));
        $this->assertEquals(0, my_extension_add(-5, 5));
        $this->assertEquals(100, my_extension_add(50, 50));
    }

    public function testAddFunctionWithInvalidType()
    {
        $this->expectException(TypeError::class);
        my_extension_add('string', 5);
    }

    public function testExtensionVersion()
    {
        $version = phpversion('my_extension');
    ...
    {
        $default = ini_get('my_extension.some_setting');
...
// Use Pest's more expressive syntax

it('returns correct sum', function () {
    expect(my_extension_add(2, 3))->toBe(5);
});

it('prints hello message', function () {
    ob_start();
    my_extension_hello('World');
    $output = ob_get_clean();
    
    expect($output)->toBe('Hello, World!' . PHP_EOL);
})->skip(!extension_loaded('my_extension'), 'Extension not loaded');

it('has version information', function () {
    expect(phpversion('my_extension'))->not->toBeEmpty();
});
```
...
<phpunit xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
         ...
         bootstrap="vendor/autoload/phpunit.php"
         colors="true"
         ...
         failOnWarning="true">
    <testsuites>
        <testsuite name="My Extension Tests">
...
**Running PestPHP Tests:**

```bash
# Run all PestPHP tests
cd /path/to/my_extension/tests/php
./vendor/bin/pest

# Run specific test file
./vendor/bin/pest tests/php/PestTest.php
...
**PestPHP Tips:**
- Use `->skip()` for conditional test skipping
- Use `expect()` for fluent assertions
- Use `it()` for more readable test descriptions
- Use Higher-Order Testing for more expressive tests
- Use `--filter` to run specific test patterns

**Combining PHPT and PestPHP:**

Both test types serve different purposes:

| Aspect | PHPT Tests | PestPHP Tests |
|--------|------------|---------------|
| Level | C-level extension testing | Userland PHP testing |
| Speed | Slower (requires PHP rebuild) | Faster (pure PHP) |
| Coverage | Core extension logic | API usage patterns |
| Setup | Requires PHP source tree | Works anywhere with Composer |
| Best for | Low-level functionality | Integration testing |

**Recommended Workflow:**
1. Write PHPT tests for core extension functionality
2. Write PestPHP tests for userland API usage
3. Run PHPT tests: `make test` in extension directory
4. Run PestPHP tests: `./vendor/bin/pest` in tests/php directory
